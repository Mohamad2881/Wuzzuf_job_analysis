...
//        return Services.getData();
    ...
    public String kMeans(Model model){
        List<Object> result = service.kMeans();
        List<Row> indexedCols = (List<Row>) result.get(0);
        double accuracy= (double) result.get(1);
        Vector[] centers = (Vector[]) result.get(2);
        model.addAttribute("indexedCols", indexedCols);
